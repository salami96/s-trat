...
<html lang="pt-br">
  <head>
    ...
    <title>Adicionar Medições</title>
...
-->              <li class="nav-item">
              ...
              <li class="nav-item">
                <a class="nav-link active" href="../prof/medicao">
...
-->              <li class="nav-item">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
          <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Adicionar Lembrete de Medições</h1>
          </div>
          <form class="needs-validation" action="../medicoes/" method="POST" novalidate>
            ...
            <label  class="h5" for="medicamento">Medição *</label>
            <select id="medicao" class="form-control"></select>
            <div class="alert alert-danger mt-2" id="erroMed" role="alert" hidden>O campo medição é obrigatório!</div>
            <label class="h5" for="obs">Observações</label>
            <input type="text" class="clear form-control" id="obs" name="obs" placeholder="Medir em jejum / Termômetro debaixo do braço ...">
            <div class="row">
    ...
    $(document).ready(function() {
      ...
      $.get('../medicoes', function(resp){
        for(i in resp){
            $('#medicao').append("<option value='" + resp[i].idMedicoes +"'>" + resp[i].descricao + "</option>");
        }
      });
      ...
      $('#add').click(function(){
        if(validate()){
          ...
          nomeMed = $("#medicao option:selected").html()
          $('#receitas').append(
              "<tr id='"+id+"'>"+
                "<th scope='row'>"+id+"</th>"+
                "<td>"+nomeMed+"</td>"+
          ...
          $('#modal-body').append(
            "<p id='p_"+id+"'>Medir: "+nomeMed+"</br>Dur: "+duracao+", "+vezes+" vez(es) por dia. "+obs+"</p>"
          );
          idMed = $("#medicao option:selected").val()
          json[id] = {"pac":$("#pac").val(),"medicao":idMed,"obs":$("#obs").val(),"hora":$("#horaIni").val()+':00',"data":$("#dataIni").val(),"vezes":vezes,"tratContinuo":tratC,"duracao":nDias,"dias":dias,"idProf":<?php echo $_SESSION["idProf"]?>};
      ...
      $('#send').click(function(){
        for(i in json){
          if((i != 0)&&(json[i] != "")){
            $.post("../medicoes",json[i],function(result){
