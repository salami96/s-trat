...
<html lang="pt-br">
  <head>
    ...
    <title>Adicionar Procedimentos</title>
...
-->              <li class="nav-item">
              ...
              <li class="nav-item">
                <a class="nav-link active" href="../prof/proceds">
...
-->              <li class="nav-item">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
          <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Adicionar Lembrete de Procedimentos</h1>
          </div>
          <form class="needs-validation" action="../procedimentos/" method="POST" novalidate>
            ...
            <label  class="h5" for="procedimento">Procedimento *</label>
            <input type="text" class="clear form-control" id="procedimento" name="procedimento" placeholder="Pegar sol / Tomar água / Caminhada...">
            <div class="alert alert-danger mt-2" id="erroProced" role="alert" hidden>O campo procedimento é obrigatório!</div>
            <label class="h5" for="obs">Observações</label>
            <input type="text" class="clear form-control" id="obs" name="obs" placeholder="3 repetições / 10 min ...">
            <div class="row">
          ...
          <div class="text-center">
            ...
            <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Impressão da Lista</h5>
                    <button type="button" class="close close-modal" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
    ...
    $(document).ready(function() {
      var id = 1;
      var duracao, proc, dias, nDias, tratC;
      if($(window).width()<=768){
        collapsar();      
        $('main').addClass('main');
      ...
      function validate(){
        var valid = false;
        if(($('#procedimento').val() != null)&&($('#procedimento').val() != "")){
          proc = $('#procedimento').val();
          valid = true;
        }else{
          var valid = false;
          $('#erroProced').fadeIn().attr('hidden', false);
        }
      ...
      $('#add').click(function(){
        if(validate()){
          ...
          feather.replace();
          var dose = "";
          var doseJ = "Nenhuma Obs.";
          if ($('#obs').val() != "") {
            dose = "Obs: "+$('#obs').val();
            doseJ = $('#obs').val();
          }
          $('#modal-body').append(
            "<p id='p_"+id+"'>"+proc+"</br>Dur: "+duracao+", "+vezes+" vez(es) por dia. "+dose+"</p>"
            );
          var imp;
          json[id] = {"pac":$("#pac").val(),"procedimento":proc,"obs":doseJ,"hora":$("#horaIni").val()+':00',"data":$("#dataIni").val(),"vezes":vezes,"tratContinuo":tratC,"duracao":nDias,"dias":dias,"idProf":<?php echo $_SESSION["idProf"]?>};
      ...
      $('#send').click(function(){
        for(i in json){
          if((i != 0)&&(json[i] != "")){
            $.post("../procedimentos/",json[i],function(resp){
              if(resp['status'] == 'true'){
                $('#post').html("<div role='alert' class='alert alert-success'>"+resp['msg']+"</div>");
              }else{
                $('#post').html("<div role='alert' class='alert alert-danger'>"+resp['msg']+" "+resp['erro']+"</div>");
              }
              feather.replace();
            });
          }
